...
fn main() {
    // `one foot`が幽霊型`Inch`を持つように明示する。
    let one_foot: Length<Inch> = Length(12.0, PhantomData);
    // `one meter`が幽霊型`Mm`を持つように明示する。
    let one_meter: Length<Mm> = Length(1000.0, PhantomData);

    // 以下の`+`は上で定義した`Length<Unit>`用の`add()`メソッドを呼び出す。
    // `Length`は`Copy`トレイトを持っているため、`add()`は`one foot`及び`one
    // meter`を消費する代わりにそのコピーを作り、`self`、`rhs`として扱う。
    ...
    // 異なる単位間の加算は意味を成さないので、以下はコンパイルエラーになる。
